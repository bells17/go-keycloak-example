class SamlController < ApplicationController
  ...
  def sp_logout_request
    ...
    if settings.idp_slo_target_url.nil?
      ...
      logger.info "New SP SLO fo User ID: '#{session[:nameid]}', Transaction ID: '#{session[:transaction_id]}'"
  ...
  def process_logout_response
    ...
    if not logout_response.validate
      ...
      if logout_response.success?
        logger.info "Delete session fo '#{session[:nameid]}'"
        reset_session
      end
    end
  end
  ...
  def idp_logout_request
    ...
    logger.info "IdP initiated Logout fo #{logout_request.nameid}"
